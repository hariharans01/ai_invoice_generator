  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-smeibold text-slate-900">All Invoices</h1>
    ...
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm">
      ...
      {filteredInvoices.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-slate-500 mb-6 max-w-md">Your search of filter criteria did not match any invoices. Try adjusting your search.</p>
          {invoices.length === 0 && (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end gap-2" onClick={(e)=> e.stopPropagation()}>
                      <Button
                        ...
                        {invoice.status !== 'Paid' && (
                          <Button size="small" variant="ghost" onClick={()=>handleOpenReminderModel(invoice._id)} title="Generate Reminder"><Mail className="w-4 h-4 text-blue-500" /></Button>
                        )}
                    </div>
